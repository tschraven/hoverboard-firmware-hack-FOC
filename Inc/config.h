...
#define N_MOT_MAX       125             // [rpm] Maximum motor speed limit
...
#define DEFAULT_FILTER              13000  // Default for FILTER 0.1f [-] lower value == softer filter [0, 65535] = [0.0 - 1.0].
    ...
    #define DEFAULT_STEER_RATE          90
    #define DEFAULT_SPEED_RATE          40

    // Per-axis base low-pass filter (fixdt(0,16,16))
    #define DEFAULT_STEER_FILTER        24000
    #define DEFAULT_SPEED_FILTER        24000
...
*/
  ...
  #define PRI_INPUT1            2,  1569, 1977, 2442, 80      // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #define PRI_INPUT2            2,  1603, 1959, 2362 , 80      // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #ifdef DUAL_INPUTS
  ...
  #else
    #define FLASH_WRITE_KEY     0x1337    // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h Original value 0x1001
    #define DEBUG_SERIAL_USART3           // right sensor board cable, disable if I2C (nunchuk or lcd) is used!
  #endif
