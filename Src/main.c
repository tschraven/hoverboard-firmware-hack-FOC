...
{
  if (speedAbs_rpm <= STEER_SOFT_RPM_START)  return 32767;
  if (speedAbs_rpm >= STEER_SOFT_RPM_FULL)   return STEER_SOFT_MIN_GAIN_Q15;

  int32_t span   = (int32_t)STEER_SOFT_RPM_FULL - STEER_SOFT_RPM_START;
  int32_t delta  = (int32_t)speedAbs_rpm       - STEER_SOFT_RPM_START;
  int32_t t_q15  = (delta << 15) / span;             // 0..1 in Q15
  ...
  while(1) {
    ...
    // ---------------------------------------------------------------
    {
      ...
      */
      ...
      // Only apply cap if rider is actually asking for forward torque
      int16_t speedCmdAbs = (speed >= 0) ? speed : (int16_t)(-speed);
      if (speedCmdAbs > SPEED_CAP_CMD_NEUTRAL) {
      ...
      // else: throttle is near neutral → cap is effectively off; e-brake is free to work
      #endif
